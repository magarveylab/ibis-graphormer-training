...
class BiosyntheticWindowOrfGraphDataGenerator:
    ...
    def add_meta_to_graph(self, prism_fh: str):
        # Reminder to you that the metadata has already been assigned for these graphs.
        pass

    ###################################
    # Multilabel management functions #
    ###################################
    def get_node_multilabel_lengths(self, label_types=List[str]):
        out = {x: 0 for x in label_types}
        for n in self.G.nodes():
            node_props = self.G.nodes[n]
            for label_type in label_types:
                labels = node_props.get(label_type)
                if labels is None:
                    labels = []
                num_labels = len(labels)
                if num_labels > out[label_type]:
                    out[label_type] = num_labels
        return out

    def add_multi_labels(
        self,
        array: np.ndarray,
        labels: List[str],
        lookup: Dict[str, int],
        ...
        if labels is not None:
            ...
            if len(labels) != 0:
                label_ints = [
                    x
                    for x in [lookup.get(label) for label in labels]
                    if x is not None
                ]
                if len(label_ints) > 0:
                    # remember, start at 1 b/c 0th position indicates the number of classes for that label type.
                    for label_idx, label_int in enumerate(label_ints, start=1):
                        array[index][label_idx] = label_int
        # if none of the labels are used, ignore the data point for prediction.
        if np.all(array[index] == 0):
            array[index].fill(self.ignore_index)

    def assign_graph_multilabel_from_multi_label(
        self, tensor: torch.tensor, is_dense: bool
    ):
        if is_dense:
            # slice out the 0th position which states # labels.
            num_labels = tensor[0, 0]
            assert torch.all(tensor[:, 0] == num_labels)
            G_tensor = torch.unique(tensor[:, 1:])
            G_tensor = G_tensor[torch.where(G_tensor != self.ignore_index)]
            if len(G_tensor) == 0:
                G_tensor = torch.LongTensor([num_labels, self.ignore_index])
            else:
                G_tensor = torch.cat(
                    (torch.LongTensor([num_labels]), G_tensor)
                )
        else:
            G_tensor = torch.amax(tensor, dim=0)
            if torch.all(G_tensor == 0):
                G_tensor = torch.full_like(
                    G_tensor, fill_value=self.ignore_index
                )
        return torch.LongTensor(G_tensor)
